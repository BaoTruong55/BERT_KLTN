...
args = parser.parse_args("""
    --test_path {baseDir}/Data/data_1/test.csv
    ...
    --rdrsegmenter_path {baseDir}/VnCoreNLP/VnCoreNLP-1.1.1.jar
""".format(baseDir=BASE_DIR).split())
bpe = fastBPE(args)
...
def nomalize_data_comment_vnexpress(commentId, comments):
    df = pd.DataFrame({
        "id": commentId,
        "text": comments,
        "data_text": comments
...
def predict_data(df):
    ...
    for fold in range(5):
        ...
        model_bert.load_state_dict(torch.load(
            os.path.join(args.ckpt_path, f"model_{fold}.bin")))
        ...
        for i, (x_batch,) in pbar:
            y_pred = model_bert(
                x_batch.cuda(), attention_mask=(x_batch > 0).cuda())
